...
class PurchaseOrderRepositoryPostgresql(IPurchaseOrderRepository):
    ...
    async def save(self, purchase_order: PurchaseOrder) -> None:
        query = """INSERT INTO purchase_orders
                ...
                """
        args = (str(purchase_order.id), str(purchase_order.disc.id), str(purchase_order.customer.id),
                purchase_order.quantity, purchase_order.timestamp, dumps(purchase_order.customer.to_json()),
